...
# for the same reason. Keep it in step with that copy. The one delta:
#
#   - the in-cluster DNS pattern ('.svc.cluster.local') is dropped. This
#     chart's server certificate names every Service of the cluster fully
#     qualified (<cluster>-{rw,ro,r}.<namespace>.svc.cluster.local), so the
#     template, its test and the README contain the suffix by design:
#     mechanism, not a particular. A concrete leak still trips the
#     account-id and hostname patterns below.
set -uo pipefail
